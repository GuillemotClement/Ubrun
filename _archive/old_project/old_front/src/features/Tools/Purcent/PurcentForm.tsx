...
export default function PurcentForm({ setResult }: PurcentFormProps) {
  const form = useForm({
    ...
    onSubmit: ({ value }) => {
      ...
      const resultValue = calculatorTools.getPurcentValue(numericValue, numericPurcent);
  ...
  return (
    ...
    >
      ...
      <form.Field name="value">
        {(field) => <FormField field={field} isRequired={true} label="Valeur" type="number" />}
      </form.Field>

      <form.Field name="purcent">
        {(field) => <FormField field={field} isRequired={true} label="Pourcentage" type="number" />}
      </form.Field>
